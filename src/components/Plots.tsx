    ...
    return Object.entries(data).map(([year, values]) => (
        <Accordion title={year} key={year}>
            ...
            {values.map((x, i) => (
                ...
                >
                    <div
                        className="transition-color absolute bottom-0 left-0 right-0 top-auto hidden rounded-t-md border border-black bg-yellow-200 text-center duration-75 hover:border-yellow-300 hover:bg-yellow-300 lg:block"
                        style={{
                            height: `${(x / Math.max(...values)) * 100}%`,
                        }}
                    >
                        {x > 0 && (
                            <p className="absolute -top-6 left-0 right-0 hidden select-none text-center font-medium leading-5 lg:inline-block">
                                {x}
                            </p>
                        )}
                    </div>
                    ...
                    >
                        {x > 0 && (
                            <p className="absolute -right-5 bottom-0 top-0 select-none text-center font-medium leading-5 lg:hidden">
                                {x}
                            </p>
                        )}
                    </div>
